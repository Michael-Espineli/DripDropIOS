...
final class AuthenticationViewModel: ObservableObject{
    ...
    func signUpWithEmail(email:String,password:String,firstName:String,lastName:String,company:String,position:String) async throws{
        let companyId = UUID().uuidString
        let authDataResult = try await AuthenticationManager.shared.createUser(email: email, password: password)
        let userId = authDataResult.uid
        sleep(1)
        let user = DBUser(id: userId, email: authDataResult.email, photoUrl: authDataResult.photoUrl, dateCreated: Date(),firstName: firstName, lastName: lastName,accountType: "Buisness",exp: 0)
        ...
        try await AuthenticationManager.shared.uploadCompany(company: Company(id: companyId, ownerId: userId, name: company))
    ...
    func signUpWithEmailFromInviteCode(email:String,password:String,firstName:String,lastName:String,company:String,position:String,invite:Invite) async throws{
        ...
        let userId = authDataResult.uid
        sleep(1)
        let user = DBUser(id: userId, email: authDataResult.email, photoUrl: "https://firebasestorage.googleapis.com/v0/b/the-pool-app-3e652.appspot.com/o/duck128.jpg?alt=media&token=549d29cd-0565-4fa4-a682-3e0816cd2fdb", dateCreated: Date(),firstName: firstName, lastName: lastName,accountType: "Technician", exp: 0)
