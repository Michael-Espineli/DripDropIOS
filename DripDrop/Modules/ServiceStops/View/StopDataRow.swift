...
struct StopDataRow: View {
    ...
    init(dataService:any ProductionDataServiceProtocol,stopData:StopData,readingTemplates:[ReadingsTemplate],dosageTemplate:[DosageTemplate]) {
    ...
    @State var readingTemplates:[ReadingsTemplate]
    @State var dosageTemplate:[DosageTemplate]
    ...
    var readings: some View {
        HStack{
            ForEach(readingTemplates){ reading in
                ...
                let amount:String = stopData.readings.first(where: {$0.templateId == reading.id})?.amount ?? "0"
    ...
    var dosages: some View {
        HStack{
            ForEach(dosageTemplate){ dosage in
                ...
                if let dosage:Dosage = stopData.dosages.first(where: {$0.templateId == dosage.id}) {
                    Text("\(name)")
                        .font(.footnote)
                        .frame(width: 50)
                        .foregroundColor(Color.clear)
