...
struct StopDataReadingInputView: View {
    ...
    var body: some View {
                            ...
                            HStack(spacing: 8){
                                ...
                                ForEach(template.amount,id:\.self){ amount in
                                    Button(action: {
                                        ...
                                        if let reading = stopData.readings.first(where: {$0.templateId == template.id && $0.bodyOfWaterId == bodyOfWaterId}) {
        ...
        .onChange(of: stopData, perform: { datum in
            if let reading = datum.readings.first(where: {$0.templateId == template.id && $0.bodyOfWaterId == bodyOfWaterId}) {
        ...
        .onChange(of: input, perform: { change in
            if let dosage = stopData.readings.first(where: {$0.templateId == template.id}) {
                ...
                stopData.readings.removeAll(where: {$0.templateId == template.id})
