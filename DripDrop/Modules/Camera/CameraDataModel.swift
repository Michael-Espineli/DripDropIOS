...
final class CameraDataModel: ObservableObject {
    ...
    @Published var uploadingImages: [UIImage] = []
    ...
    func convertPhotoAssetsToImages(photos:[PhotoAsset]) async {
        print("")
        print("Trying to Convert Photos To Images")

        self.uploadingImages = []
        for photo in photos {
            await photoCollection.cache.requestImage(for: photo, targetSize: CGSize(width: 256, height: 256))  { result in
                if let result = result {
                    Task { @MainActor in
                        self.thumbnailImage = result.image
                        if let image = result.image {
                            ...
                            if let uiImage = renderer.uiImage {
                                self.uploadingImages.append(uiImage)
