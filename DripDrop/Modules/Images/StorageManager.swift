...
final class StorageManager {
    ...
    private func userReference(user:DBUser)->StorageReference {
        storage.child("users").child(user.id)
    }
    private func customerReference(user:DBUser)->StorageReference {
        storage.child("customers").child(user.id)
    }
    private func jobRefrences(user:DBUser)->StorageReference {
        storage.child("jobs").child(user.id)
    }
    private func repairRequestRefrences(requestId:String)->StorageReference {
        storage.child("repairRequest").child(requestId)
    }
    ...
    //                            CREATE
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    func saveRepairRequestImage(companyId:String,requestId:String,data:Data) async throws ->(path:String, name:String) {
        ...
        print("meta >> \(meta)")
            ...
            let returnedMetaData = try await repairRequestRefrences(requestId: requestId).child(path)
