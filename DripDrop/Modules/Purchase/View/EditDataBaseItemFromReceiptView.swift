        ...
        VStack{
            ...
            ScrollView{
                ...
                Button(action: {
                    ...
                    Task{
                        try? await viewModel.addDataBaseItem(companyId: company.id, dataBaseItem: DataBaseItem(id: UUID().uuidString, name: pushName, rate: Double(pushRate) ?? 0.00, storeName: pushStoreName ?? "Unknown", venderId: pushStoreId, category: pushCategory,subCategory: subCategory, description: pushDescription, dateUpdated: pushDateUpdated, sku: pushSku, billable: pushBillable,color: "",size: "",UOM: UOM))
                        newItemView = false
                    }
